...
type EditableStoredElement = ReturnType<typeof findElement> & {
  rotation: number;
  opacity: number;
};

function findEditableElement(
  design: Parameters<typeof findElement>[0],
  elementId: string,
): EditableStoredElement {
  return findElement(design, elementId) as EditableStoredElement;
}
...
test('선택 요소를 정렬·회전·투명도 편집·복제하고 Undo한다', async ({ page }) => {
  ...
  const beforeTitle = findEditableElement(before, 'title');
  const beforeCount = before.pages[0]?.elements.length ?? 0;

  await page.getByRole('button', { name: '캔버스 왼쪽에 맞춤' }).click();
  await expect.poll(async () => findEditableElement(await readSavedDesign(page, cardId), 'title').x)
    .toBeCloseTo(0, 4);

  const rotation = page.getByRole('spinbutton', { name: '회전 각도' });
  await rotation.fill('30');
  await rotation.press('Tab');
  await expect.poll(async () => findEditableElement(await readSavedDesign(page, cardId), 'title').rotation)
    .toBe(30);
  ...
  await expect.poll(async () => findEditableElement(await readSavedDesign(page, cardId), 'title').opacity)
    .toBe(0.6);
  ...
  const duplicated = (await readSavedDesign(page, cardId)).pages[0]?.elements
    ...
    .find((element) => element.id !== 'title') as EditableStoredElement | undefined;
