...
function patchObject(object: FabricObject, element: DesignElement): void {
  ...
  if (element.type === 'text' && object instanceof Textbox) {
    const editingSameText = object.isEditing && object.text === element.text;
    object.set({
      ...(editingSameText ? {} : { text: element.text }),
      width: element.width,
      fontFamily: APPROVED_FONT_FAMILIES.has(element.fontFamily) ? element.fontFamily : 'system-ui',
    ...
    if (!editingSameText) object.initDimensions();
  } else if (element.type === 'shape' && element.shape === 'rectangle' && object instanceof Rect) {
    object.set({ width: element.width, height: element.height, fill: element.fill, scaleX: 1, scaleY: 1 });
  } else if (element.type === 'shape' && object instanceof Ellipse) {
    object.set({ rx: element.width / 2, ry: element.height / 2, fill: element.fill, scaleX: 1, scaleY: 1 });
    ...
    object.set({ width: element.width, height: element.height, scaleX: 1, scaleY: 1 });
  }

  object.setCoords();
}

function sameOrder(current: FabricObject[], next: FabricObject[]): boolean {
  return current.length === next.length && current.every((object, index) => object === next[index]);
}

function disposeBestEffort(objects: Iterable<FabricObject>): void {
  for (const object of objects) {
    try {
      object.dispose();
    } catch {
      // Removed or superseded runtime objects must not make a committed document mutation fail.
    }
  }
}
...
export class FabricEditorRenderer implements EditorRenderer {
  ...
  async render(design: Design): Promise<void> {
    ...
    if (!this.isCurrent(generation, canvas)) {
      disposeBestEffort(prepared.filter((entry) => !entry.reused).map((entry) => entry.object));
      return;
    }
    ...
    const retiredObjects = canvas.getObjects().filter((object) => !nextObjectSet.has(object));
    if (retiredObjects.length > 0) {
      canvas.remove(...retiredObjects);
      visualChange = true;
    }

    const mountedObjects = new Set(canvas.getObjects());
    for (const object of nextObjects) {
      if (!mountedObjects.has(object)) {
        canvas.add(object);
        visualChange = true;
      }
    }

    if (!sameOrder(canvas.getObjects(), nextObjects)) {
    ...
    disposeBestEffort(retiredObjects);
  }
