...
describe('Design migration pipeline', () => {
  ...
  it('실제 v1 문서를 v2로 single-hop migration한다', () => {
  ...
  it('v1에서 허용되던 중복 ID는 순서와 데이터를 유지하며 결정적으로 고유화한다', () => {
    const current = createSampleDesign();
    const duplicate = structuredClone(current.pages[0].elements[0]);
    duplicate.id = current.pages[0].elements[0].id;
    const duplicatePage = structuredClone(current.pages[0]);
    const v1 = {
      ...current,
      version: 1,
      pages: [
        { ...current.pages[0], elements: [...current.pages[0].elements, duplicate] },
        { ...duplicatePage },
      ],
    };
  ...
  it('미래 버전과 비정상 이전 버전은 현재 클라이언트에서 unsupported로 보존한다', () => {
    expect(migratePersistedDesign({ version: 3 })).toEqual({
